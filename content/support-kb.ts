...
- The pre-order build is free and complete and needs no account. Paid tiers arrive at launch with a 14 day free trial in the app: Protein Plan (reroll any single meal into a different dish from what's already on the list, receipt reveal, the weekly solve challenge) and Courier (reroll opens a menu of dishes with the same protein to choose from, the whole list delivered from your store, every Sunday at 5pm a reminder, with next week's plan ready when the app opens). Courier arrives in an update after release. Exact prices are in the pricing facts below.
