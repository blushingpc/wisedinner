...
export const site = {
  ...
  include: {
    ...
    items: [
      ...
      { title: "One short list with a real total", body: "Everything for the week, grouped by aisle, with an estimated in-store total. You also see what the same list costs delivered, estimated." },
  ...
  why: {
    ...
    items: [
      ...
      { title: "Shelf prices, not app prices", body: "We quote what the shelf says and show what the same list costs delivered, estimated, next to it. Delivery apps mark items up 15 to 25 percent before fees.", icon: "tag" },
  ...
  pricing: {
    ...
    tiers: [
      ...
      },
      {
        ...
        perMonth: 8,
        popular: false,
        lead: "Reroll opens a menu of dishes that fit your list and hit the same protein as the meal you're replacing, so you choose. Order the whole list delivered from your store. Every Sunday at 5pm we remind you, and next week's plan is ready when you open the app.",
        rows: ["Everything in Protein Plan", "Choose your replacement from a menu, same protein as the meal it replaces", "Delivery from your store, straight from the list", "Next week ready every Sunday at 5pm, with a reminder"],
        tag: "After release",
        note: "Arrives in an update after release.",
      },
    ] as { name: string; tagline: string; monthly: number; yearly: number; perMonth: number; popular: boolean; lead: string; rows: string[]; tag?: string; note?: string }[],
  },
