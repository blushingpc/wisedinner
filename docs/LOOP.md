...
3. Verify with your own eyes: `node scripts/journey.ts --out design/shots/iter-<N>` (needs `npm run dev` on :3077 or `--base <url>`). It screenshots `/` at 390×844 and 1440×900, runs the full quiz ($55 / 150 g), asserts `/plan` shows a feasible receipt, screenshots `/plan` `/drop` `/pricing`, clicks every nav + footer link and asserts 200s, drives the waitlist form against a mocked `/api/waitlist` (page.route) and asserts the /thanks state renders — no real row is ever written, no keys needed. Then LOOK at every screenshot (Read each PNG) and write `design/shots/iter-<N>/AUDIT.md`: blunt, one line per finding, severity tagged (truth / conversion / a11y / polish / nitpick).
