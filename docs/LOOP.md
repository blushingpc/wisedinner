...
3. Verify with your own eyes: `node scripts/journey.ts --out design/shots/iter-<N>` (needs `npm run dev` on :3077 or `--base <url>`). It screenshots `/` at 390×844 and 1440×900, runs the full quiz ($55 / 150 g), asserts `/plan` shows a feasible receipt, screenshots `/plan` `/drop` `/pricing`, clicks every nav + footer link and asserts 200s, drives the waitlist form against a mocked `/api/waitlist` (page.route) and asserts the /thanks state renders, then fires one LIVE smoke POST (loop-test@wisedinner.com) at the preview's real `/api/waitlist` — the email is seeded, so anything but `{status:"already"}` is a loud failure. Then LOOK at every screenshot (Read each PNG), run `/impeccable audit` on every changed surface (fix what it flags or say why not), then the `web-design-guidelines` skill (audit role — it reviews, never designs), and write `design/shots/iter-<N>/AUDIT.md`: blunt, one line per finding, severity tagged (truth / conversion / a11y / polish / nitpick).
