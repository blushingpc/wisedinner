    ...
    <main id="main">
      <section className="py-band">
        <div className="mx-auto max-w-[720px] px-6">
          <p className="text-caption font-semibold text-kale">a solved week · shared from the app</p>
            ...
            {w.days.map((d) => (
              <li key={d.day} className="grid grid-cols-[minmax(0,1fr)] gap-3 py-5 sm:grid-cols-[110px_minmax(0,1fr)]">
                <div>
                  ...
                  <p className="font-mono text-spec text-ink-soft">
          ...
          <div className="mt-6 rounded-[14px] border border-rule bg-white p-6">
            {AISLES.map((aisle) => {
              const rows = w.list.items.filter((i) => i.aisle === aisle);
              if (!rows.length) return null;
              return (
                <div key={aisle} className="mt-5 first:mt-0">
                  <p className="text-caption font-bold tracking-[0.04em] text-kale uppercase">{aisle}</p>
                  <ul className="mt-1 divide-y divide-rule">
                    {rows.map((i) => (
                      <li key={i.name} className="flex items-baseline justify-between gap-4 py-2 text-[0.9375rem]">
                        <span>{i.name}</span>
                        <span className="font-mono text-spec text-ink-soft">{usd(i.price_usd)}</span>
                      </li>
                    ))}
                  </ul>
                </div>
            ...
            <div className="mt-5 flex items-baseline justify-between border-t border-rule pt-4">
              ...
              <span className="font-mono text-[1.25rem] font-semibold">{usd(w.list.est_total_usd)}</span>
            </div>
            <p className="mt-1 font-mono text-spec font-semibold text-kale">
              {w.list.delivery_label} {usd(w.list.delivery_est_usd)} · walking in saves {usd(w.list.delivery_saves_usd)}
            </p>
          </div>
          <p className="mt-6 text-caption text-ink-soft">prices are shelf estimates with a buffer, labeled as estimates. the receipt is the proof.</p>
        </div>
      </section>
    </main>
  );
