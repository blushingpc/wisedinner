...
export function StatStrip({ stats, stacked }: { stats: [React.ReactNode, string][]; stacked?: boolean }) {
  return (
    <dl className={stacked ? "grid gap-2" : "grid gap-6 sm:grid-cols-3"}>
      {stats.map(([value, label], i) => (
        <div key={label} data-reveal style={{ "--i": i } as React.CSSProperties} className="border-t border-rule py-4">
          <dd className="font-mono text-4xl font-medium tabular-nums text-green-600 sm:text-5xl">{value}</dd>
