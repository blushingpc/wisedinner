  ...
  return (
    <section className="cv-auto py-band">
      <div className="mx-auto max-w-[1200px] px-6 lg:px-12">
        ...
        <p className="mt-2 text-caption font-semibold text-ink-2">people running the early build</p>
        <ul className="mt-8 grid gap-4 sm:grid-cols-2 lg:mt-12 lg:grid-cols-3 lg:gap-6">
          {TESTERS.map((t, i) => (
            <li key={t.name} className={`lift rounded-[14px] border border-border bg-white p-6 ${i % 3 === 1 ? "lg:mt-8" : ""}`}>
              <p className="text-[1.0625rem] leading-relaxed text-balance">“{t.quote}”</p>
              <div className="mt-5 flex items-center gap-3">
                <span aria-hidden="true" className="grid size-10 shrink-0 place-items-center rounded-[10px] bg-surface text-[0.875rem] font-bold text-forest">
