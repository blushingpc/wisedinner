...
// cards collapse to a chip row above the phone. Tabs pattern: arrow keys move, Home/End jump.
          ...
          <div className="order-2 lg:order-1">
            <div id={`${id}-panel`} role="tabpanel" aria-labelledby={`${id}-tab-${active}`} className="mx-auto w-[260px] lg:w-[340px]">
              <DeviceFrame label={`Phone showing ${items[active].title}`} widthClass="w-full" chrome={false} sizes="(min-width: 1024px) 340px, 260px">
                {screens.map((s, i) => (
                  <div key={i} aria-hidden={i !== active} className={`absolute inset-0 transition-opacity duration-[250ms] ease-out motion-reduce:transition-none ${i === active ? "opacity-100" : "pointer-events-none opacity-0"}`}>
                    {s}
                  </div>
                ))}
              </DeviceFrame>
