    ...
    <Screen>
        ...
        {AISLES.map((aisle) => {
          ...
          return (
            <div key={aisle} className="mt-[0.75em] first:mt-0">
              <p className="text-[0.6875em] font-bold tracking-[0.04em] text-forest uppercase">{aisle}</p>
              <ul className="mt-[0.3em] divide-y divide-border">
                {rows.map((i) => (
                  <li key={i.name} className="flex items-center gap-[0.6em] py-[0.45em] text-[0.875em]">
                    <span aria-hidden="true" className={`grid size-[1.25em] shrink-0 place-items-center rounded-[0.3em] border text-[0.7em] font-bold ${i.checked ? "border-forest bg-forest text-white" : "border-border bg-white"}`}>
                      {i.checked ? "✓" : ""}
                    </span>
                    <span className={`min-w-0 flex-1 truncate ${i.checked ? "text-ink-2 line-through" : ""}`}>{i.name}</span>
                    <span className="tnum text-[0.9em] text-ink-2">{usd(i.price_usd)}</span>
                  </li>
                ))}
              </ul>
            </div>
      ...
      <div className="border-t border-border bg-white px-[1.25em] pt-[0.75em] pb-[0.5em]">
        <div className="flex items-baseline justify-between">
          ...
          <span className="tnum text-[1.25em] font-semibold">{usd(list.est_total_usd)}</span>
        </div>
        <p className="mt-[0.25em] tnum text-[0.75em] font-semibold text-forest">
