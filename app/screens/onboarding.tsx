  ...
  return (
    <Screen>
      ...
      <div className="mt-[3.75em] px-[1.25em]">
        <p className="text-[0.75em] font-semibold text-ink-2">{step} of 6</p>
      ...
      <p className="mt-[1.4em] text-center tnum text-[3.5em] leading-none font-semibold tracking-tight">{value}</p>
      <div className="mx-auto mt-auto mb-[0.75em] grid w-[88%] grid-cols-3 gap-[0.5em]">
        {KEYS.map((k) => (
          <span key={k} className={`grid h-[3.1em] place-items-center rounded-[0.875em] tnum text-[1.25em] font-medium ${k === "⌫" || k === "." ? "text-ink-2" : "bg-white shadow-[0_1px_0_rgba(27,26,24,0.06)]"}`}>
            {k}
          </span>
        ))}
      </div>
