...
// S5, S1, S2, S6, S3, S4 → public/store/<n>-<screen>.png, and S7 (SITE-V5) → public/store/07-two-numbers.png.
// run with a dev or prod server up; --only renders a comma list of screens and leaves the rest alone:
//   node scripts/store-shots.ts [--base http://localhost:3077] [--only s7,s2]
...
const ORDER = ["s5", "s1", "s2", "s6", "s3", "s4", "s7"];
const NAME: Record<string, string> = { s7: "two-numbers" };
...
for (const [i, screen] of ORDER.entries()) {
  ...
  const out = `public/store/${String(i + 1).padStart(2, "0")}-${NAME[screen] ?? screen}.png`;
